"""Single-agent integration test for RCE-17 v0 climate model.

This test runs a deterministic DDPG rollout on the RCE-17 v0 environment and
...
from examples.climate_models.rce17_v0 import RadiativeConvectiveModelEnv
...
test_data = retrieve_tfrecord_data(
    "ddpg", glob.glob(f"tests/data/runs/{EXP_ID}_*/*_ddpg_*/*tfevents*")[0]
)


def test_rce17_v0_episodic_return_matches_expected():
    """Run the RCE-17 v0 loop and assert the last episodic return equals expected.
