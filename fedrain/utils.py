...
def make_env(
    ...
    **kwargs,
    ...
    render_mode : str or None, optional
        Rendering mode for the environment (default: None).
    capture_video : bool, optional
        Whether to capture video of the environment (default: False).
    capture_video_freq : int, optional
        Frequency of video capture (default: every 10 episodes).
    videos_dir : str, optional
        Directory to save videos (default: "./videos").
    run_name : str, optional
        Name of the run (used for video file naming, default: "climaterl").
    env_idx : int, optional
        Index of the environment instance (useful for video file naming, default: 0).
...
def get_timestamp():
    """
    Get the current timestamp as a string formatted for filenames.

    Returns
    -------
    str
        Current timestamp in "YYYY-MM-DD_HH-MM-SS" format.
    """
